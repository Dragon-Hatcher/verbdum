...
function verbdumForDay(date) {
  var id = verbdumIdForDay(date);
  return possible_answers_1.possibleAnswers[(id <= 8 ? id + 410 : id) % possible_answers_1.possibleAnswers.length];
}
...
var romanPattern = /^(M{1,4}(CM|CD|D?C{0,3})(XC|XL|L?X{0,3})(IX|IV|V?I{0,3})|M{0,4}(CM|C?D|D?C{1,3})(XC|XL|L?X{0,3})(IX|IV|V?I{0,3})|M{0,4}(CM|CD|D?C{0,3})(XC|X?L|L?X{1,3})(IX|IV|V?I{0,3})|M{0,4}(CM|CD|D?C{0,3})(XC|XL|L?X{0,3})(IX|I?V|V?I{1,3}))$/;
...
exports.toggleTranslation = exports.loadPreferredLanguage = exports.getCurrentNumber = exports.getCurrentTextForId = exports.translatePage = void 0;
  ...
  for (var i = 0; i < translatable.length; i++) {
    ...
    if (transId == "number") {
      if (/\d+/.test(translatable[i].textContent)) {
        if (language == "latin") translatable[i].textContent = (0, roman_numerals_1.romanize)(+translatable[i].textContent);
      } else {
        if (language == "english") translatable[i].textContent = (0, roman_numerals_1.deromanize)(translatable[i].textContent).toString();
      }
    } else {
      var text = translations_1.translations[transId][language];
      var word = window.currentlyPlayingWord;
      if (transId == "answer-was") text += "<a target=\"_blank\" rel=\"noopener noreferrer\" href=\"https://en.wiktionary.org/wiki/".concat(word.toLowerCase(), "#Latin\">").concat(word.toUpperCase(), "</a>");
...
function getCurrentNumber(number) {
  if (window.currentLanguage == "latin") {
    return (0, roman_numerals_1.romanize)(number);
  } else {
    return number.toString();
  }
}

exports.getCurrentNumber = getCurrentNumber;
...
},{"./constants/translations":"../js/constants/translations.ts","./roman-numerals":"../js/roman-numerals.ts"}],"../js/modal.ts":[function(require,module,exports) {
"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.showThanksModal = exports.showStatsModal = exports.showHelpModal = void 0;
...
document.getElementById("thanks-link").addEventListener("click", function (e) {
  e.stopPropagation();
  showThanksModal();
});
...
exports.loadGameForDate = exports.submit = exports.backspaceLetter = exports.addLetter = exports.updateStatsPage = exports.readStats = exports.isPlaying = exports.getColors = void 0;
...
function toastWithMessage(messageId) {
  ...
  var toastBag = document.getElementById("main-toast-bag");
...
function showAnswer() {
  ...
  var toast = document.createElement("div");
  var word = window.currentlyPlayingWord;
  toast.innerHTML = (0, translate_1.getCurrentTextForId)("answer-was") + "<a target=\"_blank\" rel=\"noopener noreferrer\" href=\"https://en.wiktionary.org/wiki/".concat(word.toLowerCase(), "#Latin\">").concat(word.toUpperCase(), "</a>");
  toast.classList.add("toast");
...
function guessWordInRow(word, answer, row) {
  ...
  if (word == answer) {
    setTimeout(modal_1.showStatsModal, letterFlipDelay * 5 + 550);
  } else if (row == 5) {
    setTimeout(modal_1.showStatsModal, letterFlipDelay * 5 + 550 + 1000);
  }

  window.currentWordNumber++;
  window.currentLetterNumber = 0;
  window.currentlyGuessingWord = "";
  var colors = getColors(word, answer);

  var _loop3 = function _loop3(i) {
    var letterElement = getLetterElement(row, i);
    clearLetter(letterElement, false);
    setTimeout(function () {
      return setClueForLetter(row, i, word[i], colors[i]);
    }, i * letterFlipDelay);
  };
...
function updateStatsPage() {
  ...
  document.getElementById("stat-played").innerText = (0, translate_1.getCurrentNumber)(currentStats.roundsPlayed);
  document.getElementById("stat-win-percent").innerText = (0, translate_1.getCurrentNumber)(Math.round(currentStats.roundsWon / currentStats.roundsPlayed * 100 || 0));
  document.getElementById("stat-current-streak").innerText = (0, translate_1.getCurrentNumber)(currentStats.currentStreak);
  document.getElementById("stat-max-streak").innerText = (0, translate_1.getCurrentNumber)(currentStats.maxStreak);
  ...
  for (var i = 0; i < 6; i++) {
    ...
    graph.children[0].textContent = (0, translate_1.getCurrentNumber)(solveNumbers[i]);
...
function startTimer(duration, display) {
  ...
  function timer() {
    ...
    if (diff >= 0) {
      ...
      seconds = diff % 60 | 0;
      display.children[0].textContent = (0, translate_1.getCurrentNumber)(hours);
      display.children[1].textContent = (0, translate_1.getCurrentNumber)(minutes);
      display.children[2].textContent = (0, translate_1.getCurrentNumber)(seconds);
    } else {
      display.innerHTML = "<span data-trans=\"now\">".concat((0, translate_1.getCurrentTextForId)("now"), "</span>");
...
checkShouldShowHelp();
},{"./game":"../js/game.ts","./translate":"../js/translate.ts","./modal":"../js/modal.ts"}],"../js/share.ts":[function(require,module,exports) {
"use strict";
...
var shareElement = document.getElementById("share-button");
...
function modalToastWithMessage(messageId) {
  var fades = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : true;
  var toastBag = document.getElementById("modal-toast-bag");
  var toast = document.createElement("div");
  toast.innerText = (0, translate_1.getCurrentTextForId)(messageId);
  toast.classList.add("toast");
  toast.classList.add("toast-fade");
...
function getText() {
  if (!(0, game_1.isPlaying)()) {
    var colors = window.pastGuesses.map(function (x) {
      return (0, game_1.getColors)(x, window.currentlyPlayingWord);
    });
    var emojiMap = {
      "correct": "🟩",
      "present": "🟨",
    ...
    var emojis = colors.map(function (row) {
      return row.map(function (x) {
        return emojiMap[x];
      }).join("");
    }).join("\n");
    var id = (0, roman_numerals_1.romanize)(window.currentlyPlayingId);
    var attempts = window.pastGuesses.includes(window.currentlyPlayingWord) ? (0, roman_numerals_1.romanize)(window.pastGuesses.length) : '-';
    return "verbdum ".concat(id, " ").concat(attempts, "/VI\n\n").concat(emojis);
  }

  return null;
}
},{"./game":"../js/game.ts","./roman-numerals":"../js/roman-numerals.ts","./translate":"../js/translate.ts"}],"../js/index.ts":[function(require,module,exports) {
require('./keyboard.ts');
...
require('./share.ts');
},{"./keyboard.ts":"../js/keyboard.ts","./load-todays-word.ts":"../js/load-todays-word.ts","./modal.ts":"../js/modal.ts","./share.ts":"../js/share.ts"}],"../../node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
